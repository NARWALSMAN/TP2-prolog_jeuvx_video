...
%Temporalite du joueur. Ce predicat sert a definir dans quelle timeline le joueur se trouve
...
% position du joueur. Ce predicat sera modifié au fur et é mesure de la partie (avec `retract` et `assert`)
...
decrire(lama):- temps_courant(futur),
    write("Vous arrivez dans ce vetistute laboratoire. la machine a caffeest casse "), nl,
    write("M hyvernat est dans son bureau a corriger des copie de scratch"), nl,
    !.
decrire(quattreCanton):- temps_courant(futur),
    write("vous arriver dans les tres vieux batiment de l'univ"), nl,
    ...
    write("defaillant au tp, defaillant au semestre defaillant a l'annee, on est d'accord?"), nl,
...
decrire(huitB):- temps_courant(futur),
    write("dernier grand batiment de cette universite qui n'a pas encore ete detruit"), nl,
...
decrire(polytech):- temps_courant(futur),
    ...
    write("wental est en train de develloper une applis mobile pour partir d'ici"), nl,
    !.
